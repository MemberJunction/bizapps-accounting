...
| `packages/Angular/test-harnesses/gui/` | GUI/DOM (tier 4) | Vitest + TestBed + jsdom (`vitest.gui.config.ts`, MJDEV-managed; `pnpm run test:gui` in `packages/Angular`, **opt-in, not in CI**) | the running **MJAPI** through the real GraphQL client (port and key from `mjdev`) | components render real API data at value level, without a browser |
