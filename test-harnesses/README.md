...
| `packages/Angular/src/**/*.dom.test.ts` (not here — beside each component) | Component DOM | Vitest + TestBed + jsdom (`vitest.dom.config.ts`; `pnpm run test:dom`, **in CI**) | nothing — data access is faked per spec | a component renders and reacts correctly against controlled data, with no backend |
| `packages/Angular/test-harnesses/gui/` | GUI/DOM (tier 4) | Vitest + TestBed + jsdom (`vitest.gui.config.ts`, MJDEV-managed; `pnpm run test:gui` in `packages/Angular`, **opt-in, not in CI**; resolves only inside an mjdev instance, since its imports are not in this repo's lockfile) | the running **MJAPI** through the real GraphQL client (port and key from `mjdev`) | components render real API data at value level, without a browser |
