...
--   vw_JEAuditTrail         — flattened JE + line + account + batch for drill-through (line-level company)
--   vw_ARtoGLRecon          — per effective-MONTH entry-status reconciliation (pending→batched→posted)
--   vw_DimensionPL          — revenue/expense netted by analytical dimension value
--   vw_BatchDispatchStatus  — batch lifecycle + control totals + summary-JE line count
--   vw_ForwardDatedJE       — staged forward-dated (rev-rec) JE rollup by company / entry-type
--
-- ALL detail/aggregate views exclude EntryType='BatchSummary' BY DEFAULT (plan
-- §5.6): summary JEs are the batch's GL-facing aggregate; counting them alongside
-- the member detail would double every dispatched amount.
--
...
JOIN __mj_BizAppsAccounting.GLAccount gl ON gl.ID = jel.GLAccountID
LEFT JOIN __mj.Company c ON c.ID = gl.CompanyID
WHERE je.Status IN ('Batched', 'GLPosted')   -- committed to the ledger (excludes still-editable Pending)
  AND je.EntryType <> 'BatchSummary'         -- default exclusion (plan §5.6)
GROUP BY gl.CompanyID, c.Name, gl.ID, gl.Code, gl.Name, gl.AccountType;
GO
...
LEFT JOIN __mj.Company c ON c.ID = gl.CompanyID
WHERE je.EntryType <> 'BatchSummary';        -- default exclusion (plan §5.6)
GO

CREATE OR ALTER VIEW __mj_BizAppsAccounting.vw_ARtoGLRecon AS
SELECT
    YEAR(je.EffectiveDate)                                    AS EffectiveYear,
    MONTH(je.EffectiveDate)                                   AS EffectiveMonth,
...
FROM __mj_BizAppsAccounting.JournalEntry je
WHERE je.EntryType <> 'BatchSummary'         -- default exclusion (plan §5.6)
GROUP BY YEAR(je.EffectiveDate), MONTH(je.EffectiveDate);
GO
...
LEFT JOIN __mj.Company c ON c.ID = gl.CompanyID
WHERE gl.AccountType IN ('Revenue', 'Expense')
  AND je.Status IN ('Batched', 'GLPosted')
  AND je.EntryType <> 'BatchSummary'         -- default exclusion (plan §5.6)
GROUP BY gl.CompanyID, c.Name, d.ID, d.Code, d.Name, dv.ID, dv.Code, dv.Name, gl.AccountType;
GO
...
SELECT
    ...
    b.SentAt,
    b.PostedAt,
    b.SummaryJournalEntryID,
    (SELECT COUNT(*) FROM __mj_BizAppsAccounting.JournalEntryLine sl WHERE sl.JournalEntryID = b.SummaryJournalEntryID) AS SummaryLineCount
FROM __mj_BizAppsAccounting.JournalEntryBatch b
LEFT JOIN __mj.Company c ON c.ID = b.CompanyID;
GO

---------------------------------------------------------------------------
-- vw_ForwardDatedJE — the staged rev-rec waterfall as REAL forward-dated JEs
--   (plan D15: no schedule tables; a "scheduled" entry is an ordinary JE with a
--   future EffectiveDate, swept only when a batch filter explicitly reaches it).
---------------------------------------------------------------------------
CREATE OR ALTER VIEW __mj_BizAppsAccounting.vw_ForwardDatedJE AS
SELECT
    je.CompanyID,
    c.Name                                AS CompanyName,
    je.EntryType,
    ...
    COUNT(*)                              AS EntryCount,
    SUM(ISNULL(jel.DebitAmount, 0))       AS TotalDebits,
    MIN(je.EffectiveDate)                 AS EarliestEffectiveDate,
    MAX(je.EffectiveDate)                 AS LatestEffectiveDate
FROM __mj_BizAppsAccounting.JournalEntry je
JOIN __mj_BizAppsAccounting.JournalEntryLine jel ON jel.JournalEntryID = je.ID
LEFT JOIN __mj.Company c ON c.ID = je.CompanyID
WHERE je.EffectiveDate > CAST(SYSUTCDATETIME() AS DATE)
  AND je.EntryType <> 'BatchSummary'
GROUP BY je.CompanyID, c.Name, je.EntryType, je.Status;
GO
...
pos AS (
    ...
    JOIN __mj_BizAppsAccounting.JournalEntry je ON je.ID = jel.JournalEntryID
    JOIN __mj_BizAppsAccounting.GLAccount gl ON gl.ID = jel.GLAccountID
    JOIN __mj_BizAppsAccounting.AccountingCompanyProfile acp ON acp.ID = gl.CompanyID
    WHERE jel.OriginalCurrencyCode IS NOT NULL
      ...
      AND je.Status IN ('Batched', 'GLPosted')
      AND je.EntryType <> 'BatchSummary'     -- default exclusion (plan §5.6)
    GROUP BY gl.CompanyID, acp.FunctionalCurrencyCode, jel.OriginalCurrencyCode
)
SELECT
    pos.CompanyID,
    c.Name                          AS CompanyName,
...
--   vw_DefRevRollforward  — deferred-revenue opening/additions/releases/closing per Company × month
...
--
-- ACCOUNT IDENTIFICATION: the AR / Deferred-Revenue control accounts are resolved via the
-- company-level GLAccountLink rows (plan D12 — vw_CompanyDefaultAccount below; roles
-- 'Accounts Receivable' / 'Deferred Revenue'), NOT by hardcoded code and NOT by ACP columns
-- (removed), so a company that re-points its control account stays correct.
--
...
---------------------------------------------------------------------------
-- vw_CompanyDefaultAccount — a company's ACTIVE default account per role,
--   resolved from the company-level GLAccountLink rows (plan D12). The shared
--   lookup the AR/DefRev views below join; the engine's ResolveLinkedAccount
--   walk is the runtime equivalent.
---------------------------------------------------------------------------
CREATE OR ALTER VIEW __mj_BizAppsAccounting.vw_CompanyDefaultAccount AS
SELECT
    TRY_CONVERT(UNIQUEIDENTIFIER, gal.RecordID) AS CompanyID,
    r.Name                                      AS RoleName,
    gal.GLAccountID
FROM __mj_BizAppsAccounting.GLAccountLink gal
JOIN __mj_BizAppsAccounting.GLAccountRole r ON r.ID = gal.GLAccountRoleID
JOIN __mj.Entity e ON e.ID = gal.EntityID
WHERE e.Name IN (N'Companies', N'MJ: Companies')
  AND gal.Status = 'Active'
  AND (gal.StartedAt IS NULL OR gal.StartedAt <= SYSDATETIMEOFFSET())
  AND (gal.EndedAt   IS NULL OR gal.EndedAt   >  SYSDATETIMEOFFSET());
GO
...
WITH defrev AS (
    SELECT
        gl.CompanyID,
        DATEFROMPARTS(YEAR(je.EffectiveDate), MONTH(je.EffectiveDate), 1) AS PeriodMonth,
    ...
    JOIN __mj_BizAppsAccounting.JournalEntryLine jel ON jel.JournalEntryID = je.ID
    JOIN __mj_BizAppsAccounting.GLAccount gl ON gl.ID = jel.GLAccountID
    JOIN __mj_BizAppsAccounting.vw_CompanyDefaultAccount cda
      ON cda.CompanyID = gl.CompanyID AND cda.RoleName = N'Deferred Revenue'
    WHERE jel.GLAccountID = cda.GLAccountID
      AND je.Status IN ('Batched', 'GLPosted')
      AND je.EntryType <> 'BatchSummary'     -- default exclusion (plan §5.6)
    GROUP BY gl.CompanyID, DATEFROMPARTS(YEAR(je.EffectiveDate), MONTH(je.EffectiveDate), 1)
)
SELECT
    d.CompanyID,
    c.Name                                         AS CompanyName,
    ...
    ISNULL(SUM(d.Additions - d.Releases) OVER (
        PARTITION BY d.CompanyID ORDER BY d.PeriodMonth
...
JOIN __mj_BizAppsAccounting.GLAccount gl ON gl.ID = jel.GLAccountID
JOIN __mj_BizAppsAccounting.vw_CompanyDefaultAccount cda
  ON cda.CompanyID = gl.CompanyID AND cda.RoleName = N'Accounts Receivable'
LEFT JOIN __mj_BizAppsCommon.Organization o ON o.ID = jel.CounterpartyOrganizationID
LEFT JOIN __mj.Company c ON c.ID = gl.CompanyID
WHERE jel.GLAccountID = cda.GLAccountID
  AND je.Status IN ('Batched', 'GLPosted')
  AND je.EntryType <> 'BatchSummary'         -- default exclusion (plan §5.6)
GROUP BY gl.CompanyID, c.Name, jel.CounterpartyOrganizationID, o.Name
