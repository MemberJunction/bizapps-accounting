 ...
 *
 * Modelled directly on `transfer-pending-purity.test.ts` in `@mj-biz-apps/accounting-ng`, which
 * already proved the idea in this repo: a prose rule decays the moment someone is in a hurry, so
 * the rule ships as a test that fails the build.
 *
 * This applies the same idea one level up. Everything here is UI **layer 1 or 2**
 * (`guides/UI_LAYERING_GUIDE.md` in the MJ repo), which means it may not know about routing or
 * about MJ Explorer. That single constraint is what lets the Explorer entity form, the
 * journal-entries slide-in, a standalone Angular app and a test all mount the same component.
 *
...
const FORBIDDEN_PATTERNS: Array<{ pattern: RegExp; why: string }> = [
  ...
  { pattern: /^@mj-biz-apps\/accounting-ng$/, why: 'the layer-3 package — that dependency points the wrong way' },
];
  ...
  it.each(files.map((f) => [relative(LIB, f), f]))('%s imports no routing and no Explorer', (_label, file) => {
    ...
    expect(
      ...
      `${relative(LIB, file)} breaks the layer boundary:\n  ${violations.join('\n  ')}\n` +
        ...
        'Emit an event (see je-events.ts) and let the host decide.',
    ).toEqual([]);
  });
