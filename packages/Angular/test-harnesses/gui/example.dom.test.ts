...
@Component({ standalone: true, selector: 'mjdev-tier4-smoke', template: `<div class="count">{{ count() }}</div>` })
class SmokeComponent {
  count = signal(-1);
  async load(): Promise<void> {
    const r = await new RunView().RunView({ EntityName: 'MJ: AI Models', MaxRows: 5, ResultType: 'simple' });
...
describe('tier-4 harness smoke (delete/replace with real app specs)', () => {
  ...
  it('renders real-API data through the real GraphQL client', async () => {
    ...
    expect(Number(fixture.nativeElement.querySelector('.count')?.textContent?.trim())).toBeGreaterThan(0);
  });
});
