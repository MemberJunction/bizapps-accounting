...
import { WorkspaceTabStore } from '../lib/transfer-pending/workspace-tabs/workspace-tab-store';
import { WorkspaceTab } from '../lib/transfer-pending/workspace-tabs/workspace-tabs.types';
...
function tab(id: string, overrides: Partial<WorkspaceTab<DraftPayload>> = {}): WorkspaceTab<DraftPayload> {
  return {
    Id: id,
    Label: `Tab ${id}`,
    Status: 'draft',
...
describe('WorkspaceTabStore', () => {
  let store: WorkspaceTabStore<DraftPayload>;

  beforeEach(() => {
    store = new WorkspaceTabStore<DraftPayload>();
  });
