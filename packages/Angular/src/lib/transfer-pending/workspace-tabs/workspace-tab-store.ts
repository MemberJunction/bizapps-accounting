import { WorkspaceTab, WorkspaceTabState } from './workspace-tabs.types';
 ...
 */
export class WorkspaceTabStore<TState = unknown> {
  private tabs: WorkspaceTab<TState>[] = [];
  private activeId: string | null = null;

  /** Current tabs, in strip order. Returns a copy — callers must not mutate internal state. */
  public get Tabs(): WorkspaceTab<TState>[] {
    return [...this.tabs];
  }
  ...
  public get ActiveTab(): WorkspaceTab<TState> | null {
   ...
   */
  public Open(tab: WorkspaceTab<TState>): WorkspaceTab<TState> {
  ...
  public SetStatus(id: string, status: WorkspaceTabState, rejectionReason: string | null = null): boolean {
    const tab = this.tabs.find((t) => t.Id === id);
    if (!tab) return false;
    tab.Status = status;
