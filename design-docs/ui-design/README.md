...
- Dashboards show no on-demand heavy aggregates — expensive stats are precomputed on a schedule or
  don't ship.
- **Forms adoption boundary (UPD-7, refined 2026-07-21):** the MJ entity form (always via
  `openBizDetail`) = simple one-record edits + detail viewing (children visible via related
  grids); the **workspace = creation + advanced edits**; process surfaces are always workspaces.
  Edit gating rides MJ's shipped machinery only (metadata ReadOnly, form-wide `EditMode` set
  from record status in our `*Extended` forms, permission-gated Edit) — the DB immutability
  triggers are the enforcement authority; locked records show real state verbs, never a
  disabled Save. No invented gating system (MJ has no state-conditional lock — verified
  2026-07-21).
- Company-scope doctrine: **pending Marcelo's dedicated scope planning pass** (UPD-7 item 3
  withdrawn — his model: scoping makes unselected companies not exist in the frontend, not
  mere query filters). Until then, no scope rules here.
