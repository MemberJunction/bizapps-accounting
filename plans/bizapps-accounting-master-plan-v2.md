...
> **Locked decisions:** branching `next → main`; PR #8 = cherry-pick after codegen regen; intercompany = N single-company JEs + flow link with Accounting auto-generating the balancing legs; balance snapshots deferred; minimal seed COA; no AI agents in v1; rev-rec computed upstream (Accounting persists+materializes); batch summary at account × dimension; PostgreSQL conversion is the **last** block.
