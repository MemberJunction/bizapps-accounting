...
Each block ≈ a 2-week sprint shipping a demoable increment. Owners: **all work is Marcelo's for now** — the earlier `M` = Marcelo / `MH` = Madhav split (still referenced in the per-block task labels below) is **retired**: Madhav isn't expected to contribute for the time being, though we may cherry-pick from his PR where useful. Read every `M`/`MH` label below as "Marcelo". **Definition of Done for every block:** code compiles (`npm run build` in the touched package) · §8.1 connection tags on new/changed files · relevant `docs/ARCHITECTURE.md` section updated · **every DB-invariant / hook / service behavior in the block's acceptance gate is an automated Vitest test (AD-17, §11.1) — including a raw-SQL bypass case per invariant — not a manual demo; UI and external-integration (BC sandbox) gates may stay demo where automation isn't feasible** · acceptance gate met · feature PR → `next`.
