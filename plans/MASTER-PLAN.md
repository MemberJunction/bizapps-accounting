...
|---|---|---|---|
| CA-1 | MOD-1 removed `AccountingPeriod` from the schema ("the ERP owns periods"), but Robert (2026-07-09 D4) requires a **closed-period posting guard** at both the order and JE layers. With no local period notion, there is nothing to guard against. | **Resolved: MOD-13 (2026-07-14)** — a lightweight **manual close guard** is reinstated (per-company close of a period/arbitrary span; trigger blocks new JEs into closed spans) because batch-then-BC-close leaves a **race condition**. MOD-1's core stands: no ERP-period bookkeeping; recognition stays date-driven (MOD-11); dispatched batches land in the ERP's active period. | Marcelo 2026-07-14 (revision; Robert's position concurs) ✅ |
