...
|---|---|---|---|
| CA-1 | MOD-1 removed `AccountingPeriod` from the schema ("the ERP owns periods"), but Robert (2026-07-09 D4) requires a **closed-period posting guard** at both the order and JE layers. With no local period notion, there is nothing to guard against. | **Resolved-for-now: follow Amith's removal** — the fuller verbatim (in MOD-1) is definitive: batches land in the ERP's ACTIVE period; "not our job to worry about." **NO local period guard is built.** Robert is still researching; reopen only if he overturns it ("there may be changes later"). | Marcelo 2026-07-13 (Amith doc confirmation) ✅(for now) |
