...
|---|---|---|---|
| CA-1 | MOD-1 removed `AccountingPeriod` from the schema ("the ERP owns periods"), but Robert (2026-07-09 D4) requires a **closed-period posting guard** at both the order and JE layers. With no local period notion, there is nothing to guard against. | **Resolved: follow the removal — NO local periods or close guard** (Amith's fuller verbatim in MOD-1 is definitive; MOD-13's brief manual-close reinstatement was WITHDRAWN same-day). Rationale: batch summaries lose date info anyway; the AR subledger doesn't own periods — accountants batch entries into the right periods. Future timing complexities handled when they arise. | Marcelo 2026-07-14 (final; MOD-13 withdrawn) ✅ |
...
> Also **MOD-4:** batch summary lines are NETTED per (Company × GLAccount × Dimension-combo), one
> net side — not separate Dr/Cr per side (§8.4); Company is degenerate under single-company
> batches but stays in the canonical key.
