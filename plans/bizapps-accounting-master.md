...
| **BA-D2** | **PostgreSQL native from day 1** via MJ's `pg-migrate` tooling. SQL Server compatibility maintained on a best-effort basis but PG is primary. | Aligns with `M1` from master plan. BC's longer-term direction. OSS ecosystem benefits from PG-first. |
...
| **BA-D11** | **`CurrencyExchangeRate`** (not `FxRate`) in BizAppsCommon. Pluggable provider via RegisterClass — ship `ExchangeRate-API`, `ECB`, `OpenExchangeRates`, `Manual`. Default provider: `ExchangeRate-API` (free tier). **Auto-fetch disabled by default**; deployments opt in via Scheduled Job (default cadence: weekly when enabled). | Per AN-BC discussion. Avoid mystery cron eating API budget. Clean naming for non-finance audiences. |
...
### 4.7 Currency + CurrencyExchangeRate (in BizAppsCommon)

Referenced here for completeness; full definitions live in BizAppsCommon.

```sql
__mj_BizAppsCommon.Currency
  ID UUID PK,
