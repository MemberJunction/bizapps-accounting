...
| D13 | **Account-driven routing (Marcelo 2026-08-14):** where a JE goes is decided by its lines' GL accounts' external system (`GLAccount.ExternalSystem`, matched to `ExternalAccountingSystem.Name`) — NOT by a per-batch selection. v1 handles the NON-SPLIT case only: at dispatch, resolve the summary lines' accounts to systems; exactly one system ⇒ post there; mixed systems ⇒ **loud fail** (the split-summary case is deferred, handled after non-split works). `JournalEntryBatch.ExternalAccountingSystemID` STAYS (stamped with the resolved system; informational + audit). External systems are NOT divided by company: nothing gets added to AccountingCompanyProfile, and connection resolution is by Integration alone (its single active CompanyIntegration; 0 or >1 ⇒ loud fail) — not by the batch's company. |
